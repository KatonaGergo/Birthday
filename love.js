(function(window){
    ...
    Seed.prototype = {
        ...
        clear: function() {
            var ctx = this.tree.ctx, circle = this.circle;
            var point = circle.point, scale = circle.scale, radius = 26;
            var w = h = (radius * scale);
            ctx.clearRect(point.x - w, point.y - h, 4 * w, 4 * h);
        },
        hover: function(x, y) {
            const dx = x - this.circle.point.x;
            const dy = y - this.circle.point.y;
            const r = this.circle.radius * this.circle.scale;
